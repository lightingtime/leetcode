...
4. 按判题结果分支：
   ...
   **Accepted（通过）**
   ...
   - 自动打卡（无需用户提醒）：`node ".agents/skills/lc-practice/scripts/update_state.js" checkin --seq <seq>`，重新生成训练主页「reviews/index.html」（打卡表 + 进度 + 复盘列表，数据源 progress.json）；随后将主页改动提交到 git。
...
5. 每次提交后向用户汇报：判题结果、用时/内存（如有）、下一步建议。
