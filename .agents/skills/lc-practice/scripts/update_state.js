...
function planReport(p) {
  ...
  const todayDone = done.filter(d => d.date === date).length;
  const lines = [`目标（写死）：第一遍 ${total} 题须在 ${FIRST_PASS_DEADLINE} 前刷完；第二遍 ${SECOND_PASS_DEADLINE} 前完成`];
  if (problems >= total) {
    lines.push(`第一遍已完成（${problems}/${total} 题），进入第二遍，须在 ${SECOND_PASS_DEADLINE} 前完成`);
    return lines;
  }
  const first = dates[0];
  const elapsed = first ? daysBetween(first, date) + 1 : 1;
  const totalDays = first ? daysBetween(first, FIRST_PASS_DEADLINE) + 1 : 1;
  const remaining = total - problems;
  const daysLeft = Math.max(0, daysBetween(date, FIRST_PASS_DEADLINE)); // 不含今天的剩余天数
  const needDaily = daysLeft > 0 ? remaining / daysLeft : remaining;
  const expected = Math.round((total * elapsed) / totalDays); // 按线性节奏今天应完成
  const gap = problems - expected;
  lines.push(`当前：完成 ${problems}/${total} 题，打卡 ${checkins} 天（自 ${first} 已 ${elapsed} 天），剩余 ${remaining} 题、${daysLeft} 天`);
  lines.push(`按 8.30 截止反推：需日均 ${needDaily.toFixed(1)} 题/天（今日目标 ${Math.ceil(needDaily)} 题）`);
  lines.push(`今日训练量：完成 ${todayDone} 题${todayDone >= Math.ceil(needDaily) ? '，达标 ✓' : '，未达标 ✗'}（按日均 ${needDaily.toFixed(1)} 题评估）`);
  lines.push(`进度评估：按线性节奏今天应完成约 ${expected} 题，实际 ${problems} 题（${gap >= 0 ? '超前 ' + gap : '落后 ' + -gap} 题）`);
  return lines;
}
